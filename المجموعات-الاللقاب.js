...
let madara;
try {
    madara = mongoose.model('madara');
} catch (error) {
    madara = mongoose.model("madara", madaraSchema);
}
...
let handler = async function (msg, { conn, text, command, isAdmin, isROwner, participants }) {
    try {

import { areJidsSameUser } from '@itsukichan/baileys';
import { createHash } from 'crypto';
import { canLevelUp, xpRange } from '../lib/levelling.js';
//import formatInternational from '../utils/pn.js';
...
let handler = async (m, { conn, args, usedPrefix, participants }) => {
  ...
  if (!(who in global.db.data.users)) throw '✳️ المستخدم ده مش موجود في قاعدة البيانات بتاعتي';
...
*انت في الترتيب ${usersExp.indexOf(m.sender) + 1} من إجمالي ${usersExp.length} عضو*`
.trim();
...
handler.help = ['التوب'];
handler.tags = ['core'];
handler.command = [ 'التوب','ترتيب','الترتيب'];

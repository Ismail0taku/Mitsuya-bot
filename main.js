...
import fs from 'fs'
import './config.js'; 
const MAIN_SESSIONS_DIR = global.sessions || 'sessions'; 
const SUB_BOTS_DIR = global.jadi || 'JadiBots';
const ENABLE_SUB_BOTS = global.yukiJadibts === true;
console.log(chalk.bold.green(`The bot has started working`));
...
const {
  ...
  jidNormalizedUser,
} = await import('@itsukichan/baileys');
...
global.db = new Low(new JSONFile(`storage/databases/database.json`), defaultData); 
...
global.authFile = MAIN_SESSIONS_DIR; 
...
async function reconnectSubBot(botPath) {
  if (!ENABLE_SUB_BOTS) {
    console.log(chalk.gray(`[DEBUG] Reconexión sub-bot معطلة.`));
    return;
  }
...
async function startSubBots() {
  if (!ENABLE_SUB_BOTS) {
    console.log(chalk.bold.yellow(`⚠️ The subbots are disabled by a variable ${chalk.cyan('global.yukiJadibts')} In a file  config.js.`));
    return;
  }
  
  const rutaJadiBot = join(__dirname, `./${SUB_BOTS_DIR}`);
  ...
  if (readRutaJadiBot.length > 0) {
    ...
    for (const subBotDir of readRutaJadiBot) {
      ...
      if (statSync(botPath).isDirectory()) {
        ...
        console.log(chalk.gray(`[DEBUG] '${subBotDir}' en ${SUB_BOTS_DIR} no es un directorio, saltando.`));
      }
    }
...
async function handleLogin() {
  if (conn.authState.creds.registered) {
    console.log(chalk.green('Sesión principal.'));
    return;
  }
  ...
  if (loginMethod === 'code') {
    let phoneNumber = await question(chalk.red('🔥 Enter your number(964839XXXXXX):\n'));
...
async function connectionUpdate(update) {
  ...
  if (connection === 'open') {
    console.log(chalk.yellow('The Main Bot Is Working'));
    console.log(chalk.bold.green('✅ The bot was successfully run without errors.'));
  }
  const reason = new Boom(lastDisconnect?.error)?.output?.statusCode;
  if (reason === 405) {
    if (existsSync(`./${MAIN_SESSIONS_DIR}/creds.json`)) unlinkSync(`./${MAIN_SESSIONS_DIR}/creds.json`);
    console.log(
      chalk.bold.redBright(
        `Conexión reemplazada para البوت الرئيسي، por favor espera un momento. Reiniciando...\nSi aparecen errores، vuelve a iniciar con: npm start`
      )
    );
    process.send('reset');
  }
...
case DisconnectReason.timedOut:
  conn.logger.warn(`⚠️ The connection has stopped. We will try again shortly....`);
  setTimeout(async () => {
    await global.reloadHandler(true).catch(console.error);
  }, 60 * 300); 
  break;
      ...
      case DisconnectReason.restartRequired:
        conn.logger.info(`Reinicio necesario del البوت الرئيسي، reinicia el servidor si hay problemas.`);
        await global.reloadHandler(true).catch(console.error);
        break;
      default:
        conn.logger.warn(`Desconexión desconocida del البوت الرئيسي: ${reason || ''} - Estado: ${connection || ''}`);
        await global.reloadHandler(true).catch(console.error);
        break;
    }
  }

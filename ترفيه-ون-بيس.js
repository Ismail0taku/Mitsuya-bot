//بصدق الامر ذا غريب ومعرف كيف يشتغل


import fetch from 'node-fetch';
const gameDuration = 60000;
const poin = 500;
const game = '.ونبيس';
const botname = `${global.gt}`; 
export async function handler(m, { command, text, conn }) {
    let id = m.chat;
    conn.tebakGame = conn.tebakGame ? conn.tebakGame : {};
    let currentGame = conn.tebakGame[id];
    let src;
    let poster = 'https://files.catbox.moe/1o5oud.jpg';
    try {
        const response = await fetch('https://gist.githubusercontent.com/owjdkdjxuwbnd/ce86d505436dd43bd42a308385264afa/raw/908191de60cc9959332abec98acb8f83cf57b0e4/gistfile1.txt');
        
        if (!response.ok) {
            throw new Error(`فشل جلب قائمة الأسئلة: حالة HTTP ${response.status}`);
        }
        src = await response.json();
    } catch (e) {
        console.error('Error fetching questions:', e);
        return conn.reply(m.chat, '> *◞⚠️◜ فشل جلب قائمة الأسئلة أو حدث خطأ في الشبكة.*', m);
    }  
    if (!src || src.length === 0) {
        return conn.reply(m.chat, '> *◞⚠️◜ لا تـوجـد أسـئـلـة مـتـاحـة فـي الـوقـت الـحـالـي.*', m);
    }
    if (currentGame) {
        if (!text) {
            return conn.reply(m.chat, `> *◞❕◜ هـنـاك لـعـبـة قـيـد الـتـشـغـيـل.*`, m);
        }
        
        const correctName = currentGame[1].name;
        if (text === correctName) {
            m.react('✅');
            global.db.data.users[m.sender].exp += poin;
            conn.sendButton(m.chat, `> *◞🎊◜ مـبـارك لـقـد ربـحـت حـصـلـت عـلـي: ${poin}ريـال💰*`, `> ${botname}`, poster, [[`↬⌯الـمــ↪️ـزيـد‹◝`, `${game}`]], null, null);
            clearTimeout(currentGame[3]);
            delete conn.tebakGame[id];
            
        } else if (text === 'انسحب') {
            clearTimeout(currentGame[3]);
            conn.sendButton(m.chat, `> *◞😂◜ تـم الانـسـحـاب بـنـجـاح ايـهـا الـفـاشـل الاجـابـة كـانـت: ${correctName}*`, `> ${botname}`, poster, [[`↬⌯الـمــ↪️ـزيـد‹◝`, `${game}`]], null, null);
            delete conn.tebakGame[id];
            
        } else {
            clearTimeout(currentGame[3]);
            m.react('❌');
            conn.sendButton(m.chat, `> *◞❌◜ الاجـابـة  خـاطـئـه الاجـابـه الـصـحـيحه كـانـت : ${correctName}*`, `> ${botname}`, poster, [[`↬⌯الـمــ↪️ـزيـد‹◝`, `${game}`]], null, null);
            delete conn.tebakGame[id];
        }
    } else {
        if (!text) {
            let question = src[Math.floor(Math.random() * src.length)];
            let options = [question.name];
            
            while (options.length < 4) {
                let option = src[Math.floor(Math.random() * src.length)].name;
                if (!options.includes(option)) {
                    options.push(option);
                }
            }      
            options = options.sort(() => Math.random() - 0.5);

            conn.tebakGame[id] = [m, question, 10, setTimeout(() => {
                delete conn.tebakGame[id];
                conn.sendButton(m.chat, `> *◞⏰  ◜انـتـهـي وقـت الـعـبـة الاجـابـة كـانـت: ${question.name}*`, `> ${botname}`, poster, [[`↬⌯الـمــ↪️ـزيـد‹◝`, `${game}`]], null, null);
            }, gameDuration)];

            let message = `
> ‹◝خـمـن شـخـصـيـة ون بيس↬

*┐┈─๋︩︪──๋︩︪─═⊐‹🍦›⊏═─๋︩︪──๋︩︪─┈┌*
> *↬⌯وقـت الاجــ⏳ـابـة ${(gameDuration / 1000).toFixed(2)} ثانية*
> *↬⌯الـجـائـ🎁ـزة: ${poin} نـقـاط خـبـره*
> *للانـسـحـاب اضـغـط عـلـى زر ◞انـسـحـب›*
*┘┈─๋︩︪──๋︩︪─═⊐‹🍦›⊏═─๋︩︪──๋︩︪─┈└*
`;
            const buttons = [
                [`①: ${options[0]}`, `${game} ${options[0]}`], 
                [`②: ${options[1]}`, `${game} ${options[1]}`], 
                [`③: ${options[2]}`, `${game} ${options[2]}`], 
                [`④: ${options[3]}`, `${game} ${options[3]}`], 
                [`◞انـسـحـاب🏃‍♂️◜`, `${game} انسحب`]
            ];
            if (question.img) {
                try {
                    await conn.sendButton(m.chat, message, `> ${botname}`, question.img, buttons, null, null);
                } catch (imgError) {
                    console.error('Error sending image button:', imgError);
                    await conn.sendButton(m.chat, `${message}\n\n*⚠️ فشل عرض الصورة، إليك الخيارات:*\n\n1. ${options[0]}\n2. ${options[1]}\n3. ${options[2]}\n4. ${options[3]}`, `> ${botname}`, poster, buttons, null, null);
                }
            } else {
                await conn.sendButton(m.chat, `${message}\n\n*الخيار الأفضل هو استخدام زر البدء لإظهار الصورة.*`, `> ${botname}`, poster, buttons, null, null);
            }

        } else {
            m.react('👇🏻');
            conn.sendButton(m.chat, `> *لـقـد انـتـهـت الـعـبـة ◞❕◜*`, `> ${botname}`, poster, [[`↬⌯الـمــ↪️ـزيـد‹◝`, `${game}`]], null, null);
        }
    }
}

handler.help = ['ونبيس'];
handler.tags = ['العاب'];
handler.command = ['ونبيس'];

export default handler;

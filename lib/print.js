import { WAMessageStubType } from '@itsukichan/baileys'
...
export default async function (m, conn = { user: {} }) {
  try {
    ...
    const intl = parsePhoneNumber(plusPrefixed)?.number?.international || plusPrefixed
    ...
    const me = parsePhoneNumber(myNumRaw.startsWith('+') ? myNumRaw : ('+' + myNumRaw))?.number?.international || myNumRaw

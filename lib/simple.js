...
/**
 * @type {import('@itsukichan/baileys')}
 */

import {
    makeWASocket as BaileysSocket,
    proto,
    areJidsSameUser, 
...
export function makeWASocket(connectionOptions, options = {}) {
    /**
     * @type {import('@itsukichan/baileys').WASocket | import('@itsukichan/baileys').WALegacySocket}
     */
    let conn = BaileysSocket(connectionOptions)
     ...
     * @param {import('@itsukichan/baileys').proto.WebMessageInfo} quoted - Message to reply to (optional)
     * @param {Boolean} ptt - If the file is a voice note
     * @param {Object} options - Additional options for sending media
     * @returns {Promise<import('@itsukichan/baileys').proto.WebMessageInfo>}
     */
     ...
     * @param {import('@itsukichan/baileys').proto.WebMessageInfo} quoted - The message to reply to (optional)
...
//كائن اعادة توجية
    ...
    async value(jid, text = '', quoted, options = {}) {
        let canalId = [global.idcanal, global.idcanal2].filter(Boolean) 
        let canalNombre = [global.packname, global.author].filter(Boolean)
        if (canalId.length === 0) {
             canalId = ['120363340261499294@newsletter']
             canalNombre = ['Default Channel Name']
        }
        
        const length = Math.min(canalId.length, canalNombre.length);
        ...
        if (Buffer.isBuffer(text)) {
            ...
            return conn.sendFile(jid, text, 'file', '', quoted, false, fileOptions)
        }
        
        const { ephemeralExpiration, ...messageContentOptions } = options;
        ...
        {
            quoted,
            ephemeralExpiration: ephemeralExpiration || global.ephemeral,
        })
    }
},
         sendMedia: {
    ...
    */
    ...
    async value(status) {
        ...
        sendButton: {
             ...
             * @param {import('@itsukichan/baileys').proto.WebMessageInfo} quoted
...
/**
    ...
    */
     ...
     * @param {import('@itsukichan/baileys').proto.WebMessageInfo} message - The message containing media
...
msToDate: {
   async value (ms) {
        ...
        cMod: {
             ...
             * cMod
             * @param {String} jid 
             * @param {import('@itsukichan/baileys').proto.WebMessageInfo} message 
             * @param {String} text 
             * @param {String} sender 
             * @param {*} options 
             * @returns 
...
parseMention: {
    ...
    if (jid.endsWith('@g.us')) return new Promise(async (resolve) => {
        ...
        const phoneNumberIntl1 = PhoneNumber('+' + jid.replace('@s.whatsapp.net', ''))?.number?.international || jid.replace('@s.whatsapp.net', '')
        resolve(v.name || v.subject || phoneNumberIntl1)
    ...
    const phoneNumberIntl2 = PhoneNumber('+' + jid.replace('@s.whatsapp.net', ''))?.number?.international || jid.replace('@s.whatsapp.net', '')
    return (withoutContact ? '' : v.name) || v.subject || v.vname || v.notify || v.verifiedName || phoneNumberIntl2
},
enumerable: true
},
loadMessage: {
             ...
             * @returns {import('@itsukichan/baileys').proto.WebMessageInfo}
             */
            value(messageID) {
        ...
        processMessageStubType: {
             ...
             * @param {import('@itsukichan/baileys').proto.WebMessageInfo} m 
             */
            async value(m) {
                ...
                for (const message of m) {
                    try {
                        ...
                        if (message.message?.[mtype]?.contextInfo?.quotedMessage) {
                            /**
                             * @type {import('@itsukichan/baileys').proto.IContextInfo}
                             */
                            let context = message.message[mtype].contextInfo
                            let participant = conn.decodeJid(context.participant)
                            const remoteJid = conn.decodeJid(context.remoteJid || participant)
                            /**
                             * @type {import('@itsukichan/baileys').proto.IMessage}
                             * 
                             */
...
export function smsg(conn, m, hasParent) {
    if (!m) return m
    /**
     * @type {import('@itsukichan/baileys').proto.WebMessageInfo}
     */
...
reply: {
    value(text, chatId, options = {}) { 
        ...
        return this.conn?.reply(chatId ? chatId : this.chat, text, this, newOptions);
    }
},
        copy: {
            value() {

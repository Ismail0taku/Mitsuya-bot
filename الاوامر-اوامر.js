...
let handler = async (m, { conn, usedPrefix, __dirname, text, isPrems }) => {
  try {
    let img = await (await fetch('https://files.catbox.moe/5xk41x.jpg')).buffer()
...
`.trim()
    ...
    let buttonMessage = {
      image: { url: 'https://files.catbox.moe/40ykys.jpg' },
      caption: str,
      mentions: [m.sender],
      footer: 'kakashi',
      headerType: 4,
      contextInfo: {
        ...
        externalAdReply: {
          showAdAttribution: false, // 🛑 تم التعديل: إيقاف خاصية "إعلان"
          mediaType: 1,
          title: '', // ❌ تم حذف العنوان
          thumbnail: img,
          sourceUrl: '' // ❌ تم حذف الرابط
        }
      }
    }
